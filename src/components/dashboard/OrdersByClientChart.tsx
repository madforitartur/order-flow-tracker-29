...
export function OrdersByClientChart({ orders }: OrdersByClientChartProps) {
  // Group orders by client
  const clientData = orders.reduce((acc, order) => {
    const existing = acc.find(c => c.name === order.terceiro);
    if (existing) {
      existing.value += 1;
      ...
      return (
        <div className="bg-popover border border-border rounded-lg shadow-lg p-3">
          ...
          <p className="text-xs text-muted-foreground">
            {data.quantity.toLocaleString('pt-PT')} unidades
          </p>
        </div>
      );
    }
