...
import { getUniqueClients, getUniqueFamilias } from '@/lib/orderUtils';
...
export default function Orders() {
  ...
  const { data: ordersResponse } = useOrdersQuery({
    ...
    family: familiaFilter !== 'all' ? familiaFilter : undefined,
    page: currentPage,
    pageSize,
    sort: sortField === 'dataEmissao' ? 'issue_date' : sortField === 'dataPedida' ? 'requested_date' : sortField === 'nrDocumento' ? 'doc_nr' : sortField === 'terceiro' ? 'client_name' : sortField === 'qtdPedida' ? 'qty' : 'qty_open',
    direction: sortDirection
  });

  const orders = ordersResponse?.data ?? [];
  const clients = useMemo(() => getUniqueClients(orders), [orders]);
  const familias = useMemo(() => getUniqueFamilias(orders), [orders]);

  const totalPages = Math.ceil((ordersResponse?.total ?? 0) / pageSize);

  useEffect(() => {
    setCurrentPage(1);
  }, [searchTerm, clientFilter, statusFilter, familiaFilter, pageSize]);
      ...
      <div className="space-y-4">
        ...
        <div className="flex items-center justify-between text-sm">
          <p className="text-muted-foreground">
            {ordersResponse?.total ?? 0} encomendas encontradas
          </p>
